...
const GigGenerator: React.FC<GigGeneratorProps> = ({ onCoachRequest, onNavigateToTab, language = 'EN' }) => {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `I just generated a new AI Money Machine strategy for: ${result.gigTitle}! Potential earnings: ${result.estimatedEarnings}. Find yours at GATMONEY.com`,
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl">
          <div className="space-y-6">
            ...
            <button
              ...
              className={`w-full py-5 px-4 rounded-2xl flex items-center justify-center gap-3 font-black text-white transition-all transform hover:scale-[1.02] uppercase tracking-widest ${
                loading || scanCount >= MAX_SCANS
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex flex-col gap-3">
                  <div className="flex gap-2">
                    <button 
                      onClick={handleSave}
                      disabled={saved}
                      className={`flex-[2] text-xs font-black uppercase flex items-center justify-center gap-2 py-4 rounded-xl transition-all ${
                        saved ? 'bg-emerald-500 text-white' : 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 hover:bg-slate-100'
                      }`}
                    >
